...
const totalCreatedTodos = computed(() => store.getters.totalCreatedTodos);

// Watch for changes in Vuex state to update visibility
watchEffect(() => {
  store.commit("setVisibility", visibility.value);
});
      ...
      <ul v-show="todos.length" class="flex items-center gap-3 lg:ml-auto">
        <li v-for="(filter, key) in Object.values(VisibilityFilter)" :key="key">
          <a
            :href="'#' + filter"
            :class="{
              'text-purple hover:underline underline-offset-2': visibility === filter,
            }"
          >
